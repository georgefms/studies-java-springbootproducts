...
public class ProductController {
    ...
    public ResponseEntity<List<ProductModel>> getAllProducts(){
        ...
        if (!productsList.isEmpty()){
            for(ProductModel product: productsList){
                ...
                product.add(linkTo(methodOn(ProductController.class).getSingleProduct(id)).withSelfRel());
            }
        }
    ...
    public ResponseEntity<Object> getSingleProduct(@PathVariable(value = "id") UUID id) {
        ...
        oneProduct.get().add(linkTo(methodOn(ProductController.class).getAllProducts()).withRel("Products List"));
